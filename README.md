...
- [ ] Throttling & Proxy Layer
  ...
  - If limit is within bounds, pass control via `next()` directly to the downstream reverse proxy layer (e.g., `http-proxy-middleware`).
...
- [ ] Load & Latency Benchmarking
  - Conduct stress testing using tools like `autocannon` or `k6` to verify low-latency overhead (< 2 ms) and low RAM consumption under high RPS.
